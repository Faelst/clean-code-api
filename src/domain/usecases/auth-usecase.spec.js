...
const makeSut = () => {
  ...
  const {
    loadUserByEmailSpy
  } = makeLoadUserByEmailSpy()
  const {
    tokenGeneratorSpy
  } = makeTokenGeneratorSpy()
  const {
    updateAccessTokenRepositorySpy
  } = makeUpdateAccessTokenRepositorySpy()
...
describe('Auth Usecase', () => {
  ...
  test('Should throw if any dependencies throws', async () => {
    ...
    const suts = [].concat(
      new AuthUseCase({
        loadUserByEmailRepository: makeLoadUserByEmailSpyWithError()
      }),
      new AuthUseCase({
        loadUserByEmailRepository,
        encrypt: makeEncrypterWithError()
      }),
      new AuthUseCase({
        loadUserByEmailRepository,
        encrypt,
